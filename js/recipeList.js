...
v
    FilterPatternUstensil(allRecipe,e) {// make the array for the specific search in the buttons
        for (let recipe of allRecipe){
   ...
   FilterListener(){//Listener in the searchbar
    ...
    FilterAlgo(e){//compare the word in the search bar with the recipList
        if(this.tagArray.length ==0){
            ...
            for(let j=0; j< this.allRecipe[i].ingredients.length; j++){
                let ingredientRecipe= this.Typo(this.allRecipe[i].ingredients[j].ingredient)
            ...
            this.tagFilter()
            for (let i=0; i< this.allRecipeFiltered.length; i++){
                ...
                for(let j=0; j< this.allRecipeFiltered[i].ingredients.length; j++){
                    let ingredientRecipe= this.Typo(this.allRecipeFiltered[i].ingredients[j].ingredient)
                    if ( nameRecipe.includes(this.Typo(e)) || ingredientRecipe.includes(this.Typo(e)) || descriptionRecipe.includes(this.Typo(e))) { 
    ...
    FilterInterface() {//Display or hide the card
       let cardDeck= document.getElementById("card-deck")
        for(let i=0; i<cardDeck.children.length; i++){
    ...
    arrayIngredient(recipes){//create the array with all the ingredients
        this.allRecipeIngredient=[]
    ...
    arrayUstensil(allRecipe){//create the array with all the ustensils
        this.allRecipeUstensil=[];
        for (let recipe of allRecipe){
            for (let ustensil of recipe.ustensils){
                this.allRecipeUstensil.push(this.strUcFirst(this.Typo(ustensil)))
